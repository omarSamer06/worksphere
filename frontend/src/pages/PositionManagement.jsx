...
const PositionManagement = () => {
  ...
  const startEdit = (pos) => {
    ...
    setEditForm({
      title: pos.title,
      department: pos.department?._id || '',
      manager: pos.manager?._id || '',
    });
  };
  ...
  return (
    <div className="max-w-3xl">
      <h2 className="text-base font-semibold text-gray-900 mb-6">Position Management</h2>

      {/* Create form */}
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 mb-6">
        <h3 className="text-sm font-semibold text-gray-700 mb-4">Add New Position</h3>
        {createError && (
      ...
      {loading ? <Loader /> : positions.length === 0 ? (
        ...
        <div className="flex flex-col gap-3">
          {positions.map((pos) => (
            <div key={pos._id} className="bg-white rounded-xl border border-gray-100 shadow-sm p-4">
              {editId === pos._id ? (
                    ...
                    <div className="flex gap-3 mt-1">
                      {pos.department && (
                        <span className="text-xs text-gray-500">
                          <span className="text-gray-400">Dept:</span> {pos.department.name}
                        </span>
                      )}
                      {pos.manager && (
                        <span className="text-xs text-gray-500">
                          <span className="text-gray-400">Manager:</span> {pos.manager.name}
                        </span>
                      )}
                    </div>
                  </div>
